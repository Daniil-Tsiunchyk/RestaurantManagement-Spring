...
public class OrderController {
  ...
  public String createOrder(@RequestBody OrderRequest orderRequest) {
    ...
    Tables table = tablesRepository
      .findById(orderRequest.getTable_id())
      .orElseThrow(() ->
        new RuntimeException("Стол не найден: " + orderRequest.getTable_id())
      );
    ...
    information =
      new StringBuilder(information.toString().replace("\n", "<br>"));
    ...
    if (date != null) {
      orders =
        orders
          .stream()
          .filter(order ->
            order.getStartTime().toLocalDateTime().toLocalDate().equals(date)
          )
          .collect(Collectors.toList());
    }
  ...
  public String viewOrdersManager(
    @RequestParam(required = false) String status,
    @RequestParam(required = false) @DateTimeFormat(
      iso = DateTimeFormat.ISO.DATE
    ) LocalDate date,
  ...
  public String updateStatusManager(
    @PathVariable int id,
    @RequestParam String status
  ) {
    Optional<Order> optionalOrder = orderRepository.findById(id);
